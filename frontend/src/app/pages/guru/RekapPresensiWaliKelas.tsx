...
const isWaliClassAttendance = (record: AttendanceRecordItem) =>
  !record.mapel || record.mapel.trim() === '';

export default function RekapPresensiWaliKelas() {
  ...
  useEffect(() => {
    void Promise.all([
      ...
      .then(([attendanceItems, kelasItems, profile]) => {
  ...
  const waliClassOptions = useMemo(
    () =>
      Array.from(
        new Map(
          schoolClasses
    ...
    [schoolClasses, teacherName]
  );

  const waliClassNames = useMemo(
    () => new Set(waliClassOptions.map((item) => item.nama)),
    [waliClassOptions]
  );
  ...
  const visibleRecords = useMemo(
    () =>
      records.filter(
        (record) => waliClassNames.has(record.kelas) && isWaliClassAttendance(record)
      ),
    [records, waliClassNames]
  );

  const bulanOptions = useMemo(
    () => uniqueValues(visibleRecords.map((record) => monthLabelFromDate(record.tanggal))),
    [visibleRecords]
  );

  const hasSelectedFilters = Boolean(selectedKelas && selectedBulan);
  ...
  }, [hasSelectedFilters, selectedBulan, selectedKelas, visibleRecords]);
  ...
  const handleDownloadExcel = () => {
    ...
    anchor.href = url;
    anchor.download = `rekap-presensi-wali-${filenamePart(selectedKelas)}-${filenamePart(selectedBulan)}.xls`;
            ...
            <tbody className="divide-y divide-gray-200">
              {!hasSelectedFilters && (
                <tr>
                  ...
                  >
                    Pilih kelas dan bulan terlebih dahulu untuk menampilkan rekap presensi.
                  </td>
                </tr>
              )}
